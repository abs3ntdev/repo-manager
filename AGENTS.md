...
Repos live at `$REPO_BASE_DIR/<host>/<owner>/<repo>/`. Base resolves as: `$REPO_BASE_DIR` > `$XDG_PROJECTS_DIR/repos` > `$HOME/repos`. Bare repo in `.bare/`, worktrees as branch-named subdirectories.
